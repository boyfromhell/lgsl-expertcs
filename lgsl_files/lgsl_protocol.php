...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_type_list()
  {
    ...
    "csgo"          => "Counter-Strike Global Ofesnive",    
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_software_link($type, $ip, $c_port, $q_port, $s_port)
  {
    ...
    "ventrilo"      => "http://ventrilowiki.com/",
...
//------------------------------------------------------------------------------------------------------------+
 ...
 function lgsl_query_97(&$server, &$lgsl_need, &$lgsl_fp) 
  { 
    require_once("class_serv/ts3admin.class.php"); 
...
$test = $tsAdmin->serverInfo();
$status = $test['data']['virtualserver_status'];

if($status == "offline") { return FALSE;}  
    $server['s']['status']             = 1; 
    $server['s']['name']             =    $test['data']['virtualserver_name']; 
    $server['s']['port']          =       $test['data']['virtualserver_port']; 
    $server['s']['players']         =     $test['data']['virtualserver_clientsonline']; 
    $server['s']['playersmax']         =  $test['data']['virtualserver_maxclients'];
    $server['s']['channelcount']     =    $test['data']['virtualserver_channelsonline']; 
    $server['s']['map']             = "Teamspeak 3"; 
    $server['e']['uptime'] =               $tsAdmin->convertSecondsToStrTime(($test['data']['virtualserver_uptime']));
  
     
    $server['e']['motd']             = $test['data']['virtualserver_welcomemessage']; 
    $server['e']['platform']         = $test['data']['virtualserver_platform']; 

    $server['e']['version']         = $test['data']['virtualserver_version']; 

    /* Conexiune la internet */
    $server['e']['ping']             = $test['data']['virtualserver_total_ping'];
    $server['e']['loss']             = $test['data']['virtualserver_reserved_slots'];
...
function lgsl_query_999(&$server, &$lgsl_need, &$lgsl_fp) 
  { 
    ...
    $server['s']['playersmax']     = $general[15]; 
     ...
     if (empty($server['s']['name']))  { $server['s']['name']  = "-"; }
    // players 
...
function lgsl_query_98(&$server, &$lgsl_need, &$lgsl_fp) 
  { 
    require_once("class_serv/vent.inc.php"); 
     
    $ip = $server['b']['ip']; 
    $port = $server['b']['c_port']; 
     
    $vent = new Vent; 
    $vent->setTimeout( 400000 );        // 100 ms timeout 
    $vent->makeRequest( 2, $ip, $port); 
    $test = $vent->getResponse() ; 

    @$wow = explode("\n",convertSpecialChars($test)); 
     
    if(!$vent || sizeof($wow)<10) { return FALSE; } 
     
    for($i=0;$i<11;$i++) { 
        $temp = explode(": ",$wow[$i]); 
        $temporary[strtolower($temp[0])] = $temp[1]; 
    } 
     
    $server['s']['status']             = 1; 
    $server['s']['name']             = $temporary['name']; 
    $server['s']['password']         = $temporary['auth']; 
    $server['s']['playersmax']         = $temporary['maxclients']; 
    $server['s']['uptime']             = $temporary['uptime']; 
    $server['s']['channelcount']     = $temporary['channelcount']; 
    $server['e']['voicecodec']         = $temporary['voicecodec']; 
    $server['e']['voiceformat']     = $temporary['voiceformat']; 
    $server['e']['platform']         = $temporary['platform']; 
    $server['e']['phonetic']         = $temporary['phonetic']; 
    $server['e']['version']         = $temporary['version']; 
    $server['s']['map']             = "Ventrilo"; 
    unset($temporary); 
     
    $channel_nr = 0; 
    $client_nr    = 0; 
     
    for($i=$i+1;$i<sizeof($wow);$i++) { 
        if(substr($wow[$i],0,11) == "CLIENTCOUNT") 
        { 
            $clients = explode(": ",$wow[$i]); 
            $server['s']['players'] = $clients[1]; 
        } 
         
        if(substr($wow[$i],0,7) == "CHANNEL") 
        { 
            $data     = substr($wow[$i], 9); 
            $arr     = explode(",",$data); 
             
            for($k=0;$k<sizeof($arr);$k++) { 
                $new_arr = explode("=",$arr[$k]); 
                $server['c'][$channel_nr][$new_arr[0]] = $new_arr[1]; 
            } 
            $channel_nr++; 
        } 
        if(substr($wow[$i],0,7) == "CLIENT:") 
        { 
            $data     = substr($wow[$i], 9); 
            $arr     = explode(",",$data); 
            ...
            for($k=0;$k<sizeof($arr);$k++) { 
                $new_arr = explode("=",$arr[$k]); 
                if($new_arr[0] == "NAME") $server['p'][$client_nr]["name"] = $new_arr[1]; 
                if($new_arr[0] == "SEC") $server['p'][$client_nr]["time"] = lgsl_time($new_arr[1]); 

                $server['p'][$client_nr]["score"] = "-"; 
            } 
            $client_nr++; 
        } 
    } 
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_query_998(&$server, &$lgsl_need, &$lgsl_fp)
  {
//---------------------------------------------------------+
//  REFERENCE: http://developer.valvesoftware.com/wiki/Server_Queries
    ...
    {
      $challenge_code = isset($lgsl_need['challenge']) ? $lgsl_need['challenge'] : "\xFF\xFF\xFF\xFF";

      if     ($lgsl_need['s']) { fwrite($lgsl_fp, "\xFF\xFF\xFF\xFFTSource Engine Query\x00"); }
