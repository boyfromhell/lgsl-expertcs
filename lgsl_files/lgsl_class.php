...
$html = "<div class='container'><div class='row'><div class='col-lg-12' style='margin-top:20px;'> <div class='hpanel hblue'>         <div class='panel-heading hbuilt'> Website Error - Eroare Site</div>      <div class='panel-body'><div class='pull-left'> ".error_ro('116', '117', 'Userul si Parola de la baza de date nu au fost setate.Pentru setarea datelor intrati in', 'lgsl_config')."<br /> ".error_en('116', '117', 'The user and password database has not been set. To set the data in the edit lines', 'lgsl_config')."</div>  <div class='pull-right'><i style='color:#FC0D79;' class='fa fa-4x pe-7s-attention'> </i> </div>         </div>        <div class='panel-footer' style='color:#FC0D79'>  <b>Daca problema persista contactati Y&M stefan_rienro sau nanistefan@gmail.com  /       If the problem persists contact Y & M stefan_rienro or nanistefan@gmail.com </b> </div>     </div> </div> </div> </div>";
$html1 = "<div class='container'><div class='row'><div class='col-lg-12' style='margin-top:20px;'> <div class='hpanel hblue'> <div class='panel-heading hbuilt'> Website Error - Eroare Site</div>      <div class='panel-body'>  <div class='pull-left'>   ".error_ro('118', '', 'Nu ma pot selecta baza de date, te rugam editeaza', 'lgsl_config')."      <br />      ".error_en('118', '', 'I can not select database, please edit', 'lgsl_config')."          </div>  <div class='pull-right'><i style='color:#FC0D79;' class='fa fa-4x pe-7s-attention'> </i> </div>          </div>         <div class='panel-footer' style='color:#FC0D79'>  <b>Daca problema persista contactati Y&M stefan_rienro sau nanistefan@gmail.com  /       If the problem persists contact Y & M stefan_rienro or nanistefan@gmail.com </b> </div>     </div> </div></div></div>";
  
    $lgsl_database  = @mysql_connect(DBHOST,DBUSER,DBPASSWORD) or die($html);
    if(!$lgsl_database) { die($html);}
    $lgsl_select_db = @mysql_select_db(DBNAME, $lgsl_database) or die($html1);
    if(!$lgsl_select_db) { die($html1);}
  }

function error_ro($line1, $line2, $mes, $file){   if(empty($line2))        { $html = '[RO] '.$mes.' <em style="color:#FC0D79;">'.$file.'</em> linia <em style="color:#FC0D79;">'.$line1.'</em> ';        }else {  $html = '[RO] '.$mes.' <em style="color:#FC0D79;">'.$file.'</em> linia <em style="color:#FC0D79;">'.$line1.'</em> si <em style="color:#FC0D79">'.$line2.'</em>';        }  return $html;}
function error_en($line1, $line2, $mes, $file){   if(empty($line2))       {  $html = '[EN] '.$mes.' <em style="color:#FC0D79;">'.$file.'</em> line <em style="color:#FC0D79;">'.$line1.'</em> ';       }else {  $html = '[EN] '.$mes.' <em style="color:#FC0D79;">'.$file.'</em> line <em style="color:#FC0D79;">'.$line1.'</em> and <em style="color:#FC0D79">'.$line2.'</em>';       }  return $html; }
     ...
     function VerifyCaptcha($response)
    {
        ...
        if($res['success'] == 'true')     return TRUE;   else return FALSE;
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_query_cached($type, $ip, $c_port, $q_port, $s_port, $request, $id = NULL)
  {
    ...
    {
      $id           = intval($id);
      $mysql_query  = "SELECT * FROM `{$lgsl_config['db']['table']}` WHERE `id`='{$id}' LIMIT 1";
    ...
    else
    {
      ...
      $mysql_query  = "SELECT * FROM `{$lgsl_config['db']['table']}` WHERE `type`='{$type}' AND `ip`='{$ip}' AND `q_port`='{$q_port}' LIMIT 1";
      $mysql_result = mysql_query($mysql_query) or die(mysql_error());
      $mysql_row    = mysql_fetch_array($mysql_result, MYSQL_ASSOC);
      ...
      if (!$mysql_row)
      {
        ...
        $mysql_query  = "INSERT INTO `{$lgsl_config['db']['table']}` (`type`,`ip`,`c_port`,`q_port`,`s_port`,`cache`,`cache_time`) VALUES ('{$type}','{$ip}','{$c_port}','{$q_port}','{$s_port}','','')";
        $mysql_result = mysql_query($mysql_query) or die(mysql_error());
    ...
    $cache['o']['game_full_name']    = $lgsl_type_list[$type];
    $cache['b']['type']    = $type;
    $cache['b']['ip']      = $ip;
    ...
    {
      $cache['o']['location'] = $lgsl_config['locations'] ? lgsl_query_location_code($ip) : "";
    }

    if (empty($cache['o']['locname']))
    {
      $cache['o']['locname'] = $lgsl_config['locations'] ? lgsl_query_location_name($ip) : "";
    }
    ...
    if ($needed)
    {
      ...
      $mysql_query  = "UPDATE `{$lgsl_config['db']['table']}` SET `cache_time`='{$packed_times}' WHERE `id`='{$mysql_row['id']}' LIMIT 1";
      $mysql_result = mysql_query($mysql_query) or die(mysql_error());
      ...
      $mysql_query  = "UPDATE `{$lgsl_config['db']['table']}` SET `status`='{$cache['b']['status']}',`cache`='{$packed_cache}',`cache_time`='{$packed_times}' WHERE `id`='{$mysql_row['id']}' LIMIT 1";
      $mysql_result = mysql_query($mysql_query) or die(mysql_error());
    }
...
//EXAMPLE USAGE: lgsl_query_group( array("request"=>"sep", "hide_offline"=>0, "random"=>0, "type"=>"source", "game"=>"cstrike") )
  ...
  function lgsl_query_group($options = array())
  {
    ...
    if ($type != "") { $mysql_where[] = "`type`='{$type}'"; }
     ...
     $mysql_query  = "SELECT `id` FROM `{$lgsl_config['db']['table']}` WHERE ".implode(" AND ", $mysql_where)."  ORDER BY {$mysql_order}";
    $mysql_result = mysql_query($mysql_query) or die(mysql_error());
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_group_totals($server_list)
  {
    if (!is_array($server_list)) { $server_list = lgsl_query_group(array("request"=>"sc")); }

    $total = array("players"=>0, "playersmax"=>0, "servers"=>0, "servers_online"=>0, "servers_offline"=>0, "sloturi_on"=>0);
   ...
   $total['sloturi_on']  = $total['playersmax']-$total['players'];

      
      
      $x = $server['s']['playersmax'];
      $y = $total['sloturi_on'];
      $total['sloturi_dis'] =  $x-$y;
...
function verifica_total($value){
   global $lgsl_config;
    lgsl_database();
    $servere = @mysql_num_rows(@mysql_query("SELECT *  FROM `{$lgsl_config['db']['table']}` WHERE `type`= '$value'"));
...
function verifica_voturi_totale(){
   global $lgsl_config;
    lgsl_database();

  $servere = @mysql_num_rows(@mysql_query("SELECT *  FROM `{$lgsl_config['db']['table']}`"));
if($servere == "0") {   $voturi_totale = "0"; }else { $verifica_voturi = mysql_fetch_array(mysql_query("SELECT SUM(voturi) FROM `{$lgsl_config['db']['table']}`")); $voturi_totale = $verifica_voturi['SUM(voturi)']; }
 return $voturi_totale;
}
//------------------------------------------------------------------------------------------------------------+

function servere_totale(){
     global $lgsl_config;
    lgsl_database();
  $servere = @mysql_num_rows(@mysql_query("SELECT *  FROM `{$lgsl_config['db']['table']}`"));

if($servere == "0") {   $servere_totale = "0"; }else { $servere_totale = $servere; 
 }
 return $servere_totale;
}

//------------------------------------------------------------------------------------------------------------+
function verifica_voturi($value){
   global $lgsl_config;
    lgsl_database();


 $date_importate = mysql_fetch_array(mysql_query("SELECT * FROM `{$lgsl_config['db']['table']}` WHERE `ip`='$value'"));
 $voturi  = $date_importate['voturi'];
 return $voturi;
}
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_lookup_id($id) // LEGACY - DO NOT USE
  {
    ...
    $mysql_query  = "SELECT `type`,`ip`,`c_port`,`q_port`,`s_port` FROM `{$lgsl_config['db']['table']}` WHERE `id`='{$id}' LIMIT 1";
    $mysql_result = mysql_query($mysql_query) or die(mysql_error());
    $mysql_row    = mysql_fetch_array($mysql_result, MYSQL_ASSOC);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_server_misc($server)
  {
    ...
    $misc['text_location']      = lgsl_text_location($server['o']['locname']);
...
//------------------------------------------------------------------------------------------------------------+
  ...
  function lgsl_text_location($locname)
  {
    global $lgsl_config;

    return $locname ? "{$lgsl_config['text']['loc']} {$locname}" : "";
  }
...
//------------------------------------------------------------------------------------------------------------+
   function lgsl_sort_extras($server)  {    if (!is_array($server['e'])) { return $server; }    ksort($server['e']);   return $server;  }
//------------------------------------------------------------------------------------------------------------+
  function lgsl_sort_players($server)
  {
    global $lgsl_config;
    if (!is_array($server['p'])) { return $server; }
    if     ($lgsl_config['sort']['players'] == "name")  { usort($server['p'], "lgsl_sort_players_by_name");  }   elseif ($lgsl_config['sort']['players'] == "score") { usort($server['p'], "lgsl_sort_players_by_score"); }
    return $server;
  }
//------------------------------------------------------------------------------------------------------------+
...
require "geoip.inc.php";
  function lgsl_query_location_code($ip)
  {
    global $lgsl_config, $lgsl_file_path;

    if ($lgsl_config['locations'] !== 1) { return $lgsl_config['locations']; }

    $gi = geoip_open($lgsl_file_path."GeoIP.dat", GEOIP_STANDARD);
    $location = geoip_country_code_by_addr($gi, $ip);

    if (!$location) { $location = "XX"; }

    return $location;
  }

  function lgsl_query_location_name($ip)
  {
    global $lgsl_config, $lgsl_file_path;
    ...
    $gi = geoip_open($lgsl_file_path."GeoIP.dat", GEOIP_STANDARD);
    $locname = geoip_country_name_by_addr($gi, $ip);

    if (!$locname) { $locname = "XX"; }
